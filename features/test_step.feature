Feature: Test step
  The test step runs go test for the configured scope.

  Background:
    Given the package scope is "./zz_bdd_not_default/..."
    And the quality scope is "./zz_bdd_not_default/..."

  Scenario Outline: passes when tests succeed
  ...
  Scenario: passes test extra args through
    Given the output mode is agent
    And test has an extra argument of "arg1" specified
    When the gate runs steps test
    Then the step passes
